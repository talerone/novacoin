...
Alternatively, install Qt Creator and open the `taler-qt.pro` file.

An executable named `taler-qt` will be built.
...
A warning for people using the *static binary* version of taler on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of taler is linked against libdb5.3.
...
installed causes taler-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
